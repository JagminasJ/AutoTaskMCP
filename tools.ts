...
export function registerTools(server: McpServer) {
  ...
  server.tool(
    ...
    `PRIMARY TOOL for tickets by resource name. Use this when user asks for tickets assigned to a resource by name (e.g., "show me tickets assigned to John Smith", "tickets for Joey Jagminas", "all tickets assigned to resource name"). This tool AUTOMATICALLY handles everything: finds the resource by name, queries tickets by assignedResourceID, applies date filters if specified, and returns ACTUAL TICKET DETAILS. Supports filtering by due date (dueDateTime field) and status. For "open tickets" queries, use excludeStatus: [3, 5] to exclude completed (3) and closed (5) tickets. DO NOT use ticketsQueryCount - that returns only a number and cannot filter by resource name. Tickets are filtered by assignedResourceID, not resourceName.`,
    {
