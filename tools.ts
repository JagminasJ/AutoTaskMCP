...
export function registerTools(server: McpServer) {
  ...
  server.tool(
    ...
    `PRIMARY TOOL for tickets by company name. Use this when user asks for tickets from a company (e.g., "show me tickets for Company Name", "latest 5 tickets for Company Name", "recent tickets for Company Name"). This tool AUTOMATICALLY handles everything: finds the company, queries tickets, sorts by date (most recent first), and returns ACTUAL TICKET DETAILS. For "most recent" or "latest" queries, DO NOT set daysAgo - leave it undefined. The tool will return ALL tickets sorted by date (most recent first). Only use daysAgo when user explicitly mentions a time period (e.g., "last 30 days", "last month", "last year"). DO NOT use ticketsQueryCount - that returns only a number and cannot filter by company name. DO NOT enumerate or count tickets - this tool already returns the most recent tickets sorted by date. The tool handles all filtering and sorting server-side, so you get the most recent tickets directly without needing to count or paginate.`,
    {
      ...
      maxRecords: z.number().max(100).optional().describe('Number of tickets to return (default: 20, max: 100). The tool automatically sorts by date DESC, so this returns the N most recent tickets. Use smaller values (10-20) to avoid response size limits.'),
    ...
    async (input, extra) => {
      try {
        ...
        // Autotask API has a max of 500 records per query
        // Fetch the maximum to ensure we get recent tickets
        const fetchSize = 500
        const ticketBody: any = {
          filter: [{ field: 'companyID', op: 'eq', value: companyIdNum }], // Use number, not string
          maxRecords: fetchSize, // Max allowed by Autotask API
        }
        ...
        if (daysToFilter === 0) {
          ...
          console.log(`[getTicketsByCompanyName] Filtering tickets: createDate >= ${cutoffDateStr} (last ${daysToFilter} days)`)
          ...
          console.log(`[getTicketsByCompanyName] Applying default date filter: createDate >= ${defaultCutoffDateStr} (tickets from 2025 onwards)`)
        ...
        if (tickets.length > 0) {
          ...
          if (daysToFilter === 0) {
            ...
            console.log(`[getTicketsByCompanyName] Using default date filter: 2025-01-01 (will filter out tickets older than this)`)
          }
          ...
          if (beforeFilter > tickets.length) {
            const filterReason = cutoffDate 
              ? (daysToFilter !== undefined && daysToFilter > 0 
                  ...
                  : 'older than 2025-01-01 (default filter - this year only)')
              : 'with invalid dates'
        ...
        const responseData = {
          ...
          dateFilter: input.daysAgo === 0 
            ...
            : input.daysAgo !== undefined && input.daysAgo > 0 
              ? `Last ${input.daysAgo} days (filtered by createDate)` 
              : 'Default filter: tickets from 2025-01-01 onwards (this year only)',
          dateRange: truncatedTickets.length > 0 ? (() => {
            // Only use valid dates (not null, not epoch, not invalid)
            const dates = truncatedTickets.map((t: any) => {
          ...
          note: truncatedTickets.length > 0 ? (() => {
            // Get the first ticket's date (should be most recent after sorting)
            const firstTicket = truncatedTickets[0]
  ...
  server.tool(
    ...
    `PRIMARY TOOL for tickets by resource name. Use this when user asks for tickets assigned to a resource by name (e.g., "show me tickets assigned to John Smith", "tickets for Joey Jagminas", "all tickets assigned to resource name"). This tool AUTOMATICALLY handles everything: finds the resource by name, queries tickets by assignedResourceID, applies date filters if specified, and returns ACTUAL TICKET DETAILS. Supports filtering by due date (dueDateTime field) and status. CRITICAL: When user asks for "open tickets", "active tickets", or "not closed/completed", ALWAYS set excludeStatus: [3, 5] to exclude completed (3) and closed (5) tickets. The tool will also automatically filter out closed/completed tickets and ensure tickets have recent activity (last 90 days) to be considered "current". DO NOT use ticketsQueryCount - that returns only a number and cannot filter by resource name. Tickets are filtered by assignedResourceID, not resourceName.`,
    {
      ...
      status: z.number().optional().describe('Optional: Filter tickets by status ID. Common statuses: 1=New, 2=In Progress, 3=Complete, 4=Waiting, 5=Closed. Use this when user asks for a specific status.'),
      excludeStatus: z.array(z.number()).optional().describe('CRITICAL for "open tickets": Use [3, 5] to exclude completed (3) and closed (5) tickets. When user asks for "open tickets", "active tickets", or "not closed/completed", ALWAYS set excludeStatus: [3, 5]. This ensures only open/active tickets are returned.'),
      daysAgo: z.number().optional().describe('ONLY use when user explicitly mentions a time period (e.g., "last 30 days" = 30, "last month" = 30, "last year" = 365). For "most recent" or "latest" queries without a time period, DO NOT set this parameter - leave it undefined. The tool will return all tickets sorted by date (most recent first) without any date filtering.'),
    },
    async (input, extra) => {
      try {
